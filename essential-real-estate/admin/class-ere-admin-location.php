...
if ( ! class_exists( 'ERE_Admin_Location' ) ) {
	...
	class ERE_Admin_Location {
		...
		public function add_form_fields_property_city( $taxonomy ) {
            ...
            wp_nonce_field('ere_property_city_meta','ere_property_city_meta_nonce');
			...
			<div id="property-state" class="form-field term-group selectdiv ere-property-select-meta-box-wrap">
				...
				<select id="property_city_state" name="property_city_state" data-slug="0"
				        class="postform ere-property-state-ajax">
					<option selected value=""><?php esc_html_e( 'None', 'essential-real-estate' ); ?></option>
					<?php
		...
		public function save_property_city_meta( $term_id, $tt_id ) {
            ...
            if ( empty( $_POST['ere_property_city_meta_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['ere_property_city_meta_nonce'] ), 'ere_property_city_meta' ) ) {
                return;
            }
		...
		public function edit_form_fields_property_city( $term, $taxonomy ) {
			...
			<tr id="property-country" class="form-field term-group-wrap ere-property-select-meta-box-wrap">
				<th scope="row"><label
							for="property_city_country"><?php esc_html_e( 'Country', 'essential-real-estate' ); ?></label>
				</th>
				<td><select class="postform ere-property-country-ajax" id="property_city_country"
				            name="property_city_country">
                        <?php foreach ($countries as $k => $v): ?>
                            <option <?php selected($k,$property_city_country) ?> value="<?php echo esc_attr($k)?>"><?php echo esc_attr($v)?></option>
                        <?php endforeach; ?>
					</select></td>
			</tr>
			<tr id="property-state" class="form-field term-group-wrap ere-property-select-meta-box-wrap">
				<th scope="row"><label
							for="property_city_state"><?php esc_html_e( 'Province / State', 'essential-real-estate' ); ?></label>
				</th>
				<td><select data-selected="<?php echo esc_attr( $property_city_state ); ?>" data-slug="0"
				            class="postform ere-property-state-ajax" id="property_city_state"
						...
						?>
                        <?php foreach ($terms as $term): ?>
                            <option <?php selected($term->term_id,$property_city_state) ?> value="<?php echo esc_attr($term->term_id)?>"><?php echo esc_attr($term->name)?></option>
                        <?php endforeach; ?>
					</select>
                </td>
			</tr>
		...
		public function add_form_fields_property_neighborhood( $taxonomy ) {
			$default_country = ere_get_option( 'default_country', 'US' );
            $countries = ere_get_selected_countries();
            wp_nonce_field('ere_property_neighborhood_meta','ere_property_neighborhood_meta_nonce');
			?>
			<div id="property-country" class="form-field term-group selectdiv ere-property-select-meta-box-wrap">
				<label for="property_neighborhood_country"><?php esc_html_e( 'Country', 'essential-real-estate' ); ?></label>
				<select id="property_neighborhood_country" name="property_neighborhood_country"
				        class="postform ere-property-country-ajax">
                    <?php foreach ($countries as $k => $v): ?>
                        <option <?php selected($k,$default_country) ?> value="<?php echo esc_attr($k)?>"><?php echo esc_attr($v)?></option>
                    <?php endforeach; ?>
				</select>
			...
			<div id="property-state" class="form-field term-group selectdiv ere-property-select-meta-box-wrap">
				...
				<select id="property_neighborhood_state" name="property_neighborhood_state" data-slug="0"
				        class="postform ere-property-state-ajax">
					<option value=""><?php esc_html_e( 'None', 'essential-real-estate' ); ?></option>
					<?php
			...
			<div id="property-city" class="form-field term-group selectdiv ere-property-select-meta-box-wrap">
				...
				<select id="property_neighborhood_city" name="property_neighborhood_city" data-slug="0"
				        class="postform ere-property-city-ajax">
					<option value=""><?php esc_html_e( 'None', 'essential-real-estate' ); ?></option>
					<?php
		...
		public function save_property_neighborhood_meta( $term_id, $tt_id ) {
            if ( empty( $_POST['ere_property_neighborhood_meta_nonce'] ) || ! wp_verify_nonce( wp_unslash( $_POST['ere_property_neighborhood_meta_nonce'] ), 'ere_property_neighborhood_meta' ) ) {
                return;
            }
		...
		public function edit_form_fields_property_neighborhood( $term, $taxonomy ) {
			...
			<tr id="property-country" class="form-field term-group-wrap ere-property-select-meta-box-wrap">
				<th scope="row"><label
							for="property_neighborhood_country"><?php esc_html_e( 'Country', 'essential-real-estate' ); ?></label>
				</th>
				<td><select class="postform ere-property-country-ajax" id="property_neighborhood_country"
				            name="property_neighborhood_country">
                        <?php foreach ($countries as $k => $v): ?>
                            <option <?php selected($k,$property_neighborhood_country) ?> value="<?php echo esc_attr($k)?>"><?php echo esc_attr($v)?></option>
                        <?php endforeach; ?>
			...
			<tr id="property-state" class="form-field term-group-wrap ere-property-select-meta-box-wrap">
				<th scope="row"><label
							for="property_neighborhood_state"><?php esc_html_e( 'Province / State', 'essential-real-estate' ); ?></label>
				</th>
				<td><select data-selected="<?php echo esc_attr( $property_neighborhood_state ); ?>" data-slug="0"
				            class="postform ere-property-state-ajax" id="property_neighborhood_state"
				            name="property_neighborhood_state">
						<option value=""><?php esc_html_e( 'None', 'essential-real-estate' ); ?></option>
						<?php
						$terms_state = get_categories(
							array(
						...
						?>
                        <?php foreach ($terms_state as $term): ?>
                            <option <?php selected($term->term_id,$property_neighborhood_state) ?> value="<?php echo esc_attr($term->term_id)?>"><?php echo esc_attr($term->name)?></option>
                        <?php endforeach; ?>
					</select></td>
			</tr>
			<tr id="property-city" class="form-field term-group-wrap ere-property-select-meta-box-wrap">
				<th scope="row"><label
							for="property_neighborhood_city"><?php esc_html_e( 'City', 'essential-real-estate' ); ?></label>
				</th>
				<td><select data-selected="<?php echo esc_attr( $property_neighborhood_city ); ?>" data-slug="0"
				            class="postform ere-property-city-ajax" id="property_neighborhood_city"
						...
						?>
                        <?php foreach ($terms_city as $term): ?>
                            <option <?php selected($term->term_id,$property_neighborhood_city) ?> value="<?php echo esc_attr($term->term_id)?>"><?php echo esc_attr($term->name)?></option>
                        <?php endforeach; ?>
					</select></td>
			</tr>
			<?php
