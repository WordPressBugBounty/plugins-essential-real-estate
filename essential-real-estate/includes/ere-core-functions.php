...
if (!function_exists('ere_get_template')) {
    function ere_get_template($template_name, $args = array(), $template_path = '', $default_path = '')
    {
        ...
        do_action('ere_before_template_part', $action_args['template_name'],$action_args['template_path'], $action_args['located'] , $action_args['args']);

        include($action_args['located']);

	    do_action('ere_after_template_part', $action_args['template_name'],$action_args['template_path'], $action_args['located'] , $action_args['args']);
    }
}
/**
...
if (!function_exists('ere_is_agent_pending')) {
	function ere_is_agent_pending() {
		global $current_user;
		wp_get_current_user();
		$user_id = $current_user->ID;
		$agent_id = get_the_author_meta(ERE_METABOX_PREFIX . 'author_agent_id', $user_id);
		if (!empty($agent_id) && (get_post_type($agent_id) == 'agent') && (get_post_status($agent_id) == 'pending')) {
			return true;
		}
		return false;
	}
...
if (!function_exists('ere_get_measurement_units')) {
    function ere_get_measurement_units()
    {
        ...
        if ($measurement_units == 'custom') {
            $measurement_units = ere_get_option('custom_measurement_units', 'SqFt');
        }
        else
        {
        ...
        return apply_filters('ere_get_measurement_units', $measurement_units);
    }
}
if (!function_exists('ere_get_measurement_units_land_area')) {
    function ere_get_measurement_units_land_area()
    {
        ...
        if ($measurement_units == 'custom') {
	        $measurement_units = ere_get_option('custom_measurement_units_land_area', 'SqFt');
        }
        else
        {
	        if ($measurement_units === 'm2') {
	    ...
	    return apply_filters('ere_get_measurement_units_land_area', $measurement_units);
    }
}
...
function ere_get_search_form_fields_config_default() {
    return apply_filters('ere_search_form_fields_config_default',array(
        'property_status',
        'property_type',
        'property_title',
        'property_address',
...
function ere_get_google_map_skins() {
    return apply_filters( 'ere_google_map_skins', array(
        ''  => _x( 'Default', 'Google Maps Skin', 'essential-real-estate' ),
        'skin1'  => _x( 'Vanilla', 'Google Maps Skin', 'essential-real-estate' ),
        'skin2'  => _x( 'Midnight', 'Google Maps Skin', 'essential-real-estate' ),
        'skin3'  => _x( 'Grayscale', 'Google Maps Skin', 'essential-real-estate' ),
        'skin4'  => _x( 'Blue Water', 'Google Maps Skin', 'essential-real-estate' ),
        'skin5'  => _x( 'Nature', 'Google Maps Skin', 'essential-real-estate' ),
        'skin6'  => _x( 'Light', 'Google Maps Skin', 'essential-real-estate' ),
        'skin7'  => _x( 'Teal', 'Google Maps Skin', 'essential-real-estate' ),
        'skin8'  => _x( 'Iceberg', 'Google Maps Skin', 'essential-real-estate' ),
        'skin9'  => _x( 'Violet', 'Google Maps Skin', 'essential-real-estate' ),
        'skin10' => _x( 'Ocean', 'Google Maps Skin', 'essential-real-estate' ),
        'skin11' => _x( 'Dark', 'Google Maps Skin', 'essential-real-estate' ),
        'skin12' => _x( 'Standard', 'Google Maps Skin', 'essential-real-estate' ),
        'custom' => _x( 'Custom', 'Google Maps Skin', 'essential-real-estate' )
    ) );
}

function ere_get_google_map_autocomplete_types() {
    return apply_filters( 'ere_google_map_autocomplete_types', array(
        'geocode'       => esc_html__( 'Geocode', 'essential-real-estate' ),
        'address'       => esc_html__( 'Address', 'essential-real-estate' ),
        'establishment' => esc_html__( 'Establishment', 'essential-real-estate' ),
        '(regions)'     => esc_html__( 'Regions', 'essential-real-estate' ),
        '(cities)'      => esc_html__( 'Cities', 'essential-real-estate' )
    ) );
}


function ere_get_property_form_section_config()
{
    return array(
...
function ere_get_property_form_section_config_default()
{
    return array(
        'title_des',
        'location',
