...
if (!class_exists('ERE_Property')) {
    ...
    class ERE_Property
    {
        ...
        public function contact_agent_ajax()
        {
            ...
            if (!$target_email) {
                echo wp_json_encode(array('success' => false, 'message' => esc_html__('Target Email address is not properly configured!', 'essential-real-estate')));
                wp_die();
            }
            //recaptcha
            ...
            $email_subject = esc_html(sprintf(__('New message sent by %1$s using contact form at %2$s', 'essential-real-estate'), $sender_name, get_bloginfo('name')));
            ...
            $email_body .= esc_html(sprintf(__('You can contact %1$s via email %2$s', 'essential-real-estate'), $sender_name, $sender_email));
            ...
            if (wp_mail($target_email, $email_subject, $email_body, $header)) {
                echo wp_json_encode(array('success' => true, 'message' => esc_html__('Message Sent Successfully!', 'essential-real-estate')));
            } else {
                echo wp_json_encode(array('success' => false, 'message' => esc_html__('Server Error: WordPress mail function failed!', 'essential-real-estate')));
            }
            wp_die();
        }
